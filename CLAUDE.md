...
3. plan 成功後、`terragrunt destroy` を実行して AWS リソースを削除
4. destroy 成功後、ラベル付き PR は自動マージされる
...
- `type:*` + `cluster:*` ラベルが付いた PR は、plan → apply (または plan-destroy → destroy) 成功後に `gh pr merge --squash --delete-branch` で自動マージ・ブランチ削除される
